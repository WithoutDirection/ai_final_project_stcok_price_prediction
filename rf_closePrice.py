...
def define_xy(window_size, train_data):
    ...
    for i in range(window_size, len(train_data)):
        x.append(train_data.iloc[i-window_size:i, 8])
        y.append(train_data.iloc[i, 8])
    return x, y
...
def prediction(days, model, exist_data, window_size, data_df):
    ...
    plot(x=new_dates, y=predict_newday, label='new day predict')
...
predict_newday = prediction(predict_days, model, data_df.iloc[:, 8], window_size, data_df)